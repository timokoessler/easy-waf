 ...
 */
function check(data){    
    if(('enabled' in config.modules.openRedirect && !config.modules.openRedirect.enabled) || typeof config.queryUrlWhitelist == 'undefined' || (config.modules.openRedirect.excludePaths instanceof RegExp && config.modules.openRedirect.excludePaths.test(data.path))){
        return true;
    }
