...
const {dotRegex, slashRegex, brackedOpen, colon, lT} = require('./specialchars.regex');
...
const regex = new RegExp(`(${lT}${slashRegex}?script|${lT}${slashRegex}?${htmlTags}|alert(${brackedOpen}|\`)|(${brackedOpen}|=)alert|javascript${colon}|prompt${brackedOpen}|${lT}xss|confirm${brackedOpen}|url${brackedOpen}|eval${brackedOpen}|${lT}${slashRegex}?(\\\?|%3F)?xml|${lT}${slashRegex}?dialog|fetch${brackedOpen}|(navigator|document|localStorage|process)${dotRegex}\\\S|toString${brackedOpen}|${jsEvents}|${lT}\\\??import|top\\[)`, 'i');
